...
import io.kalishak.galacticraftlegacy.attachment.SpacePlayerData;
...
public record GearEquippable(GearEquipmentSlot gearSlot, Holder<SoundEvent> equipSound, Optional<ResourceKey<EquipmentAsset>> assetId, Optional<HolderSet<EntityType<?>>> allowedEntities,
                             boolean dispensable, boolean swappable, boolean equipOnInteract, boolean canBeSheared, Holder<SoundEvent> shearingSound) {
    ...
    public static GearEquippable tank(GearEquipmentSlot gearSlot, OxygenTank oxygenTank) {
    ...
    public InteractionResult swapWithGearEquipmentSlot(ItemStack stack, Player player, @Nullable Transaction tx) {
        if (!player.hasData(GalacticraftAttachments.SPACE_DATA)) return InteractionResult.PASS;

        SpacePlayerData data = player.getData(GalacticraftAttachments.SPACE_DATA);
        ResourceHandler<ItemResource> gearResourceHandler = data.getResourceHandler();
        ...
        if ((!EnchantmentHelper.has(resourceInSlot.toStack(), EnchantmentEffectComponents.PREVENT_ARMOR_CHANGE) || player.isCreative()) && !resourceInSlot.is(stack.getItemHolder())) {
            ...
            if (this.swappable) {
                try (Transaction childTx = Transaction.open(tx)) {
                    ItemResource toSwap = playerHand.getResource();

                    if (!toSwap.isEmpty()) {
                        ItemResource swapped = ResourceHandlerUtil.findExtractableResource(gearResourceHandler, itemResource -> isSameGear(itemResource, this.gearSlot), childTx);
                        boolean success = false;

                        if (swapped == null || swapped.isEmpty()) {
                            if (playerHand.exchange(toSwap, 1, childTx) > 0) {
                                success = true;
                                childTx.commit();
                            }
                        } else if (playerHand.exchange(swapped, 1, childTx) > 0) {
                            if (gearResourceHandler.insert(toSwap, 1, childTx) > 0) {
                                success = true;
                                childTx.commit();
                            }
                        }

                        if (success && swapped != null) {
                            return InteractionResult.SUCCESS.heldItemTransformedTo(swapped.toStack());
                        }
                    }
                }
            }
    ...
    public InteractionResult equipOnTarget(Player player, LivingEntity entity, ItemStack stack, @Nullable Transaction tx) {
        ...
        SpacePlayerData entityInventory = entity.getData(GalacticraftAttachments.SPACE_DATA);
        ...
        if (canBeEquippedBy(entity.getType()) && entityInventory.getResourceBySlot(this.gearSlot).isEmpty()) {
            if (!player.level().isClientSide()) {
                ...
                try (Transaction childTx = Transaction.open(tx)) {
                    ...
                    if (extracted > 0 && entityInventory.getResourceHandler().insert(resource, 1, childTx) > 0) {
                        stack.split(1);
...
//                        }
    ...
    public static class Builder {
        ...
        public Builder setAsset(ResourceKey<EquipmentAsset> asset) {
        ...
        public GearEquippable build() {
            return new GearEquippable(this.gearSlot, this.equipSound, this.assetId, this.allowedEntities, this.dispensable, this.swappable, this.equipOnInteract, this.canBeSheared, this.shearingSound);
        }
    }
}
