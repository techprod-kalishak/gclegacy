...
public class GearInventoryMenu extends AbstractContainerMenu {
    ...
    public GearInventoryMenu(int syncId, Inventory playerInventory, Player player) {
        ...
        MappedItemResourceHandler<GearEquipmentSlot> gearHandler = new MappedItemResourceHandler<>(playerData.getGearEquipment().items, GearEquipmentSlot.class, GearEquipmentSlot.CODEC);

        for (int i = 0; i < 4; i++) {
            GearEquipmentSlot slot = SLOTS_IDS[i];
            Identifier identifier = TEXTURE_EMPTY_SLOTS.get(slot);
            addSlot(new GearSlot(gearHandler, equipment, player, slot, identifier, i, 79, 8 + i * 18));
        }

        addSlot(new GearSlot(gearHandler, equipment, player, GearEquipmentSlot.MASK, EMPTY_SLOT_HELMET, 4, 125, 26));
        addSlot(new GearSlot(gearHandler, equipment, player, GearEquipmentSlot.GEAR, EMPTY_SLOT_GEAR, 5, 125, 44));
