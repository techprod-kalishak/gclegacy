...
public record AstroMinerSpecialRenderer(Tier1RocketModel model) implements SpecialVehicleRenderer<RocketRenderState, Tier1RocketModel> {
    @Override
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked<Tier1RocketModel> {
        ...
        @Override
        public AstroMinerSpecialRenderer bake(BakingContext bakingContext) {
            return new AstroMinerSpecialRenderer(new Tier1RocketModel(bakingContext.entityModelSet().bakeLayer(GalacticraftModelLayers.TIER_1_ROCKET)));
        }
