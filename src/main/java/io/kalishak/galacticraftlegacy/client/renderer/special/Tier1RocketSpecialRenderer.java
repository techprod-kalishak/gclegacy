...
public record Tier1RocketSpecialRenderer(Tier1RocketModel model) implements SpecialVehicleRenderer<RocketRenderState, Tier1RocketModel> {
    @Override
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked<Tier1RocketModel> {
        ...
        @Override
        public Tier1RocketSpecialRenderer bake(BakingContext bakingContext) {
            return new Tier1RocketSpecialRenderer(new Tier1RocketModel(bakingContext.entityModelSet().bakeLayer(GalacticraftModelLayers.TIER_1_ROCKET)));
        }
